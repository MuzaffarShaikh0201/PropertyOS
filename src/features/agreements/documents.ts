...
import type { AgreementDocType } from './types';

// Private bucket — every object lives at `<user_id>/<agreement_id>/<doc_type>.<ext>`,
...
export async function uploadAgreementDocument(
  ...
  const path = `${userId}/${agreementId}/${docType}.${ext}`;
...
export async function getAgreementDocumentSignedUrl(path: string): Promise<string> {
